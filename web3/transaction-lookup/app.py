...
_url = os.environ["INFURA_URL"] # Add your own Infura URL
_web3 = Web3(Web3.HTTPProvider(_url))


def put_account_search(account_search):
    popup("Account Info", [put_scope("popup_content")], PopupSize.LARGE)
    with use_scope("popup_content"):
        if _web3.isAddress(account_search):
            balance = _web3.eth.get_balance(account_search)
            transaction_count = _web3.eth.get_transaction_count(account_search)
            put_table(
                tdata=[
                    ["Balance", f"{_web3.fromWei(balance, 'ether'):,.0f} ether"],
    ...
    with use_scope("search"):
        put_row(
            [
                ...
                put_button(
                    "Search",
                    onclick=lambda: put_block_search(pin.pin["block_search"]),
                    outline=True,
                ),
            ],
            size="40% 10px 15%",
...
def put_block_search(block_number):
    if block_number != None:
        try:
            this_block = _web3.eth.get_block(block_number)
...
@use_scope("latest")
def put_latest_blocks():
    latest_block = _web3.eth.get_block("latest")["number"]
    blocks = []
    for i in range(10):
        block = _web3.eth.get_block(latest_block - i)
        blocks.append(
            [
                put_button(
                    block["number"],
                    onclick=lambda x=block["number"]: put_block_search(x),
                    link_style=True,
                ),
                block["difficulty"],
                put_button(
                    block["miner"],
                    onclick=lambda x=block["miner"]: put_account_search(x),
                    link_style=True,
                ),
            ]
        )
...
def put_transactions():
    ...
    put_latest_transactions()


@use_scope("results")
def put_transaction_search(transaction_hash):
    if transaction_hash != None:
        try:
            this_tx = _web3.eth.get_transaction(transaction_hash)
            with use_scope("results", clear=True):
                ...
                put_table(
                    tdata=[
                        ...
                        [
                            ...
                            put_button(
                                this_tx["to"],
                                onclick=lambda: put_account_search(this_tx["to"]),
                                link_style=True,
                            ),
                        ],
                        [
                            "Value",
                            f"{_web3.fromWei(this_tx['value'], 'ether'):,.2f} ether",
                        ],
                        ["Gas", this_tx["gas"]],
                    ],
                    header=["Properties", "Values"],
...
@use_scope("latest")
def put_latest_transactions():
    latest_transactions = []
    latest_block = _web3.eth.get_block("latest")
    for tx_hash in latest_block["transactions"][-10:]:
        tx = _web3.eth.get_transaction(tx_hash)
        latest_transactions.append(
            [
                put_button(
                    f"{tx['hash'].hex()[0:10]}...",
                    onclick=lambda x=tx["hash"].hex(): put_transaction_search(x),
                    link_style=True,
                ),
                put_button(
                    f"{tx['from'][0:10]}...",
                    onclick=lambda x=tx["from"]: put_account_search(x),
                    link_style=True,
                ),
                put_button(
                    f"{tx['to'][0:10]}...",
                    onclick=lambda x=tx["to"]: put_account_search(x),
                    link_style=True,
                ),
                f"{_web3.fromWei(tx['value'], 'ether'):,.2f} ether",
            ]
        )
    put_table(
    ...
    put_grid(
        [
            [
                ...
                put_markdown("#### Charts").onclick(lambda: put_charts()),
                put_markdown("#### Blocks").onclick(lambda: put_blocks()),
                put_markdown("#### Transactions").onclick(
                    lambda: put_transactions()
                ),
            ]
        ],
        direction="column",
...
def put_charts():
    put_markdown("## Charts")
    put_html(charts.get_block_miners()).style("width: 60vw; height: 40rem")
    put_markdown(
